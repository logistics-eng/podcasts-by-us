...
export default function App() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // 1. Generate Script as a Dialogue from backend API
      ...
      if (fullText.startsWith("ERROR: COULD NOT ACCESS LINK")) {
        alert("The AI was unable to access the content of the link provided. This often happens with shortened links (like share.google) or sites with strict anti-bot protections. Please try using the direct URL from your browser's address bar, or paste the article text directly.");
      ...
      // 2. Generate Audio (Multi-Speaker TTS)
      // Split script into chunks to stay within TTS limits and prevent cutting off
      ...
      for (const line of lines) {
        ...
        if (processedLine.startsWith('Alex:')) {
          currentSpeaker = 'Alex';
        } else if (processedLine.startsWith('Sam:')) {
          currentSpeaker = 'Sam';
        } else if (hostCount === 'two') {
          // Force every line to have a speaker tag to prevent voices from bleeding into each other
          processedLine = `${currentSpeaker}: ${processedLine}`;
        }
      ...
      for (let i = 0; i < chunks.length; i++) {
          ...
          try {
            ...
            if (mimeType.includes('pcm') || mimeType.includes('l16')) {
              ...
              for (let j = 0; j < binaryString.length; j++) {
                pcmData[j] = binaryString.charCodeAt(j);
              }
            ...
            if ((error?.message?.includes('429') || error?.status === 429) && retryCount < maxRetries) {
              ...
              console.warn(`Rate limit hit. Retrying in ${waitTime}ms... (Attempt ${retryCount}/${maxRetries})`);
        ...
        if (!success) {
          throw new Error("Failed to generate audio after multiple retries due to rate limits.");
        }
      }
      ...
      if (error?.message?.includes('429') || error?.status === 429) {
        alert("The AI is currently receiving too many requests (Quota Exhausted). Please wait a minute and try again. This is a limit of the free Gemini API tier.");
      } else {
  ...
  const copyVocabToClipboard = () => {
    navigator.clipboard.writeText(vocabularyChart);
    setVocabCopied(true);
    setTimeout(() => setVocabCopied(false), 2000);
  };
              ...
              <div className="space-y-4">
                <div className="flex p-1 bg-gray-100 rounded-xl">
                  <button 
                    ...
                    className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${sourceType === 'subject' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                  >
                    Subject
                  </button>
                  <button 
                    onClick={() => setSourceType('article')}
                    className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${sourceType === 'article' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                  >
                    Article
                  </button>
                </div>
                ...
                {sourceType === 'subject' ? (
                  <>
                    <div className="space-y-2">
                      ...
                      <input 
                        type="text"
                        placeholder="e.g. The history of jazz, Quantum computing..."
                  ...
                  <div className="space-y-4">
                    <div className="flex gap-2">
                      <button 
                        onClick={() => setArticleSourceType('text')}
                        className={`flex-1 py-1.5 text-[10px] font-bold rounded-lg border transition-all ${articleSourceType === 'text' ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-white border-gray-200 text-gray-400 hover:text-gray-600'}`}
                      >
                        Paste Text
                      </button>
                      <button 
                        onClick={() => setArticleSourceType('url')}
                        className={`flex-1 py-1.5 text-[10px] font-bold rounded-lg border transition-all ${articleSourceType === 'url' ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-white border-gray-200 text-gray-400 hover:text-gray-600'}`}
                      >
                        Article URL
                      </button>
                    </div>
                    ...
                    {articleSourceType === 'text' ? (
                      <div className="space-y-4">
                        <div className="space-y-2">
                          ...
                          <textarea 
                            placeholder="Paste your first article content here..."
                            className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all min-h-[120px] resize-none"
                            value={articleText}
                        ...
                        <div className="space-y-2">
                          ...
                          <textarea 
                            placeholder="Paste your second article content here..."
                            className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all min-h-[120px] resize-none"
                            value={articleText2}
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    type="text"
                    placeholder="e.g. innovation, synergy, paradigm shift..."
                    className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                    value={specificWords}
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <label className="text-sm font-medium text-gray-600 flex items-center gap-1">
                      <Gauge size={14} /> Generator Speech Speed
                    </label>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <label className="text-sm font-medium text-gray-600 flex items-center gap-1">
                      <Users size={14} /> Narrators
                    </label>
                  </div>
                  <div className="flex bg-gray-100 rounded-xl p-1">
                    <button
                      onClick={() => setHostCount('one')}
                      className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${hostCount === 'one' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                      One Host
                    </button>
                    <button
                      onClick={() => setHostCount('two')}
                      className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${hostCount === 'two' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                      Two Hosts
                    </button>
                  </div>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-600 flex items-center gap-1">
                    <BarChart size={14} /> English Level
                  </label>
            ...
            <AnimatePresence mode="wait">
              {!transcript && !isGenerating ? (
                <motion.div 
                  ...
                  className="h-full flex flex-col items-center justify-center text-center p-12 border-2 border-dashed border-gray-200 rounded-3xl text-gray-400"
                >
                ...
                >
                  ...
                  {audioUrl && (
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center gap-4">
                      <button 
                        onClick={togglePlay}
                        className="w-14 h-14 bg-indigo-600 rounded-full flex items-center justify-center text-white hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 shrink-0"
                      >
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <a 
                          href={audioUrl} 
                          download={`Podcast-${generatedTitle.replace(/\s+/g, '-')}.wav`}
                    ...
                    <div className="p-2 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                      <div className="flex gap-1">
                        <button 
                          ...
                          className={`px-4 py-2 text-xs font-bold rounded-xl transition-all ${activeTab === 'transcript' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                        >
                          Transcript
                        </button>
                        <button 
                          onClick={() => setActiveTab('vocabulary')}
                          className={`px-4 py-2 text-xs font-bold rounded-xl transition-all ${activeTab === 'vocabulary' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                        >
                          Vocabulary Chart
                        </button>
                      </div>
                      <button 
                        onClick={activeTab === 'transcript' ? copyToClipboard : copyVocabToClipboard}
                        className="flex items-center gap-1.5 text-xs font-bold text-indigo-600 hover:bg-indigo-100 px-3 py-1.5 rounded-lg transition-all"
                      >

...
async function startServer() {
  ...
  app.post('/api/generate-script', async (req, res) => {
    try {
      ...
      const defaultHost1 = language === 'turkish' ? 'Elif' : language === 'spanish' && spanishDialect === 'argentina' ? 'Valentina' : language === 'spanish' ? 'Isabel' : language === 'french' ? 'Camille' : language === 'arabic' ? 'Layla' : language === 'italian' ? 'Giulia' : language === 'dutch' ? 'Fleur' : 'Alex';
      const defaultHost2 = language === 'turkish' ? 'Mehmet' : language === 'spanish' && spanishDialect === 'argentina' ? 'Matías' : language === 'spanish' ? 'Alejandro' : language === 'french' ? 'Lucas' : language === 'arabic' ? 'Omar' : language === 'italian' ? 'Marco' : language === 'dutch' ? 'Joost' : 'Sam';
...
IMPORTANT:
1. Start your response with a short, catchy title for this podcast episode on the first line, formatted as "TITLE: [Your Title]".${language !== 'english' ? ` Write the title in ${language === 'arabic' ? 'Arabic' : language === 'spanish' ? 'Spanish' : language === 'french' ? 'French' : language === 'italian' ? 'Italian' : language === 'dutch' ? 'Dutch' : 'Turkish'} first, then on the next line write "TITLE_EN: [English translation of the title]".` : ''}
...
${language === 'italian'
  ...
  : (level === 'A1' || level === 'A2')
    ? (language === 'arabic'
      ...
      : `3. For each vocabulary item, provide a simple explanation in ${language === 'spanish' ? 'Spanish' : language === 'french' ? 'French' : language === 'turkish' ? 'Turkish' : language === 'dutch' ? 'Dutch' : 'English'} and then a Hebrew translation of the word/phrase itself (not the explanation), in this format: "Word/Phrase = Explanation — תרגום עברי של המילה עצמה".`)
    : (language === 'arabic'
...
Keep the conversation natural and engaging. Do not include any stage directions or non-spoken text.`;
      ...
      const finalUserPrompt = language === 'spanish'
        ...
        : language === 'italian'
        ? userPrompt + '\n\nIMPORTANT: Write the ENTIRE podcast script in Italian. All dialogue, vocabulary chart, and content must be in Italian.'
        : language === 'dutch'
        ? userPrompt + '\n\nIMPORTANT: Write the ENTIRE podcast script in Dutch. All dialogue, vocabulary chart, and content must be in Dutch.'
        : userPrompt;
...
${transcript}`;
  ...
  app.post('/api/generate-audio', async (req, res) => {
    try {
      ...
      const VOICE_FEMALE_IT = 'it-IT-ElsaNeural';
      const VOICE_MALE_IT   = 'it-IT-DiegoNeural';
  ...
  app.post('/api/generate-worksheet', async (req, res) => {
    try {
      ...
      const langName = language === 'spanish' ? 'Spanish' : language === 'french' ? 'French' : language === 'arabic' ? 'Arabic (Levantine)' : language === 'turkish' ? 'Turkish' : language === 'italian' ? 'Italian' : language === 'dutch' ? 'Dutch' : 'English';
...
- Return ONLY the complete HTML document, nothing else. Start with <!DOCTYPE html>`;
  ...
  // TTS for a single word (Vocabulary Builder)
  app.post('/api/tts-word', async (req, res) => {
    try {
      const { word, voice } = req.body;
      const tts = new MsEdgeTTS();
      await tts.setMetadata(voice || 'ar-SY-AmanyNeural', OUTPUT_FORMAT.AUDIO_24KHZ_48KBITRATE_MONO_MP3);
